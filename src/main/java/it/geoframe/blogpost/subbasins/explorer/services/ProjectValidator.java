...
public final class ProjectValidator {
    ...
    public static ValidationResult validate(ProjectConfig cfg) {
        ...
        if (!errors.isEmpty()) return new ValidationResult(false, info, errors, warnings);
    ...
    private static void validateSqlite(Path sqlitePath, List<String> info, List<String> errors, List<String> warnings) {
        ...
        try (Connection c = DriverManager.getConnection("jdbc:sqlite:" + sqlitePath)) {
            ...
            if (!cols.contains("timestep")) warnings.add("SQLite.measurements: 'timestep' column missing? (should be required)");
        } catch (SQLException e) {
            errors.add("SQLite: cannot open/read DB: " + e.getMessage());
        }
    }

    private static void validateGeoPackageSqliteSide(Path geopkgPath, List<String> info, List<String> errors, List<String> warnings) {
    ...
    private static void validateGeoToolsOpen(Path geopkgPath, List<String> info, List<String> errors, List<String> warnings) {
        ...
        try {
            ...
            if (ds != null) {
                try { ds.dispose(); } catch (Exception ignored) {}
            }
        }
    }

    // --- helpers ---
    private static void requireTable(Connection c, String name, String label, List<String> info, List<String> errors) throws SQLException {
    ...
    private static void requireColumns(String label, Set<String> actual, Set<String> required,
                                       List<String> info, List<String> errors) {
        boolean ok = true;
        ...
        if (ok) info.add("✅ " + label + " columns OK: " + required);
    }
